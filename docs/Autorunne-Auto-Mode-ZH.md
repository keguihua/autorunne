# Autorunne 自动识别 / 自动初始化 / 自动恢复（0.6.6）
...
## 现在的落地方式
### 1. `autorunne open`
`autorunne open` 依然是进入仓库的核心入口；0.6.6 这次把“状态层”进一步补成了更完整的工作流 CLI：旧 workspace 可迁移、status 读 state、task 可显式操作、恢复提示会自动忽略 `.vscode/` 这类噪音。
...
## 当前阶段最重要的事
0.6.6 这一轮已经把“状态层 / 视图层 / skill 接入层 / wrapper 接入层 / legacy migration / 显式 task 操作”打通了。

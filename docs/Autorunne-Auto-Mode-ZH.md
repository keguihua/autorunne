# Autorunne 自动识别 / 自动初始化 / 自动恢复（0.6.15）
...
## 现在的落地方式
### 1. `autorunne open`
`autorunne open` 依然是进入仓库的核心入口；到 0.6.15，状态层、视图层、direct agent 入口、版本升级提示、多包项目扫描、轻量 Python 项目识别和全 agent repo 入口已经形成一条可演示的主路径。旧 workspace 可迁移，status 读 state，task 可显式操作，恢复提示会自动忽略 `.vscode/` 这类噪音。
...
- 监听本地文件变化
- 检测到变化后自动 `sync`
- 如果当前还没有活跃 task，会先自动补一条聚焦中的任务
- 然后自动写一条 checkpoint 风格的变更记录
...
## Wrapper 现在也会自动拉起 daemon
这一版开始，`ar-codex / ar-claude / ar-hermes` 这些 repo wrapper 不只是先 `open` 仓库，
还会在背后自动拉起一个后台 daemon。

这意味着：
- 用户只要进入 repo 继续开发
- 不需要自己判断什么时候 `start / checkpoint`
- 只要发生真实文件变更，Autorunne 就会自动记录
...
## 当前阶段最重要的事
到 0.6.15，Autorunne 已经把“状态层 / 视图层 / skill 接入层 / wrapper 接入层 / legacy migration / 显式 task 操作 / direct agent / 多包项目扫描 / 轻量 Python 识别 / Cursor 和 Copilot 入口”打通成一条主路径。

当前版本的关键补强是：
- watcher / daemon 一旦检测到真实文件变更，就会自动补 task + checkpoint 风格记录
- `ar-codex / ar-claude / ar-hermes` wrapper 会自动拉起后台 daemon，用户只管进仓库继续做
- `autorunne ingest`：支持 Codex / Claude Code / Hermes 这类 direct agent 入口写入自然语言任务
...
- `autorunne sync` 能识别 frontend/backend/contracts/apps/packages 这类多包 Node/TypeScript 项目，不再因为根目录缺少 package.json 回退 generic

下一步更适合继续加的是：
- JSON 输出模式，方便 wrapper / demo / agent 直接消费
- 更强的 release / changelog / publish 自动化
- 更复杂 monorepo 的跨包依赖图谱

# Autorunne 自动识别 / 自动初始化 / 自动恢复（0.6.2）
...
## 现在的落地方式
### 1. `autorunne open`
`autorunne open` 依然是进入仓库的核心入口；0.6.2 这次真正把 Autorunne 从“直接改 markdown 的 CLI”升级成了“state-first 的开发状态层”。
...
## 当前阶段最重要的事
0.6.2 这一轮已经把“状态层 / 视图层 / skill 接入层 / wrapper 接入层”打通了。

下一步更适合继续加的是：
- 更强的状态 diff 摘要
- 更细的 user-level integration
- 更强的 release / changelog 自动化

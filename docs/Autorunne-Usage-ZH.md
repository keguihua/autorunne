...
## 2. 0.5.0 核心升级
- 新增 `autorunne open`，进入仓库时可自动识别并接管半成品项目
- 如果 `.autorunne/` 不存在，会自动 bootstrap 本地工作流文件
- 如果 `.autorunne/` 已存在，会自动 refresh 并恢复工作状态
- 新增 `autorunne daemon`，让本地仓库保持“打开即续做”的常驻循环
- 新增 `autorunne hermes-task`，让 Hermes 聊天入口能直接把任务写进 `.autorunne/`
- VS Code 的 folder open 自动任务改为直接执行 `autorunne open`
- 新增项目阶段识别、最近 git 变化识别、恢复提示（resume hint）
...
## 4. 安装
### 最快安装方式（适合你在 VS Code 终端里直接输入一行命令）
```bash

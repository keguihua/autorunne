...
## 2. 0.6.9 核心升级
- direct agent 模式现在是默认产品形态：用户直接打开 Codex / Claude Code / Hermes 发任务即可
- 新增 `autorunne ingest`，让 agent 把新的自然语言任务写进 `.autorunne/`，不用再假装用户在跟 Autorunne 单独聊天
- 生成的 repo skill / START_HERE / AGENTS / Copilot / Cursor 指令都改成“Autorunne 在后台维护状态和记忆，wrapper 只是可选兜底入口”
- `.autorunne/state/*` 继续作为唯一事实源
- `status` 仍然优先读取 state，能直接看到 active task / last action / task counts / integration 状态

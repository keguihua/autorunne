...
这样 Claude Code、Codex、Gemini 这些窗口不需要“凭聊天记忆硬记住项目”，而是直接读取 `.autorunne/`。
...
### 1）只靠聊天上下文
聊天窗口一换、token 一压缩、上下文就漂。

### 2）项目状态不落地
模型知道“刚刚做了什么”，但 repo 里没有持久记录。

### 3）多个模型之间不能交接
Claude Code 看不见 Codex 的内部记忆，Gemini 也看不见 Claude 的内部记忆。
...
> Autorunne 是一个本地优先、轻量、可交接的 AI 开发工作流内核。它让 Claude Code、Codex、Gemini 这类 coding agent 在同一个仓库里共享项目记忆、任务状态和收尾流程。

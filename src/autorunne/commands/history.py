...
from autorunne.core.state_engine import session_history, workflow_exists, workflow_needs_migration
...
def run(target: Path, *, limit: int = 20) -> dict:
    ...
    if workflow_needs_migration(repo_root):
        raise RuntimeError("autorunne history found a legacy markdown workspace. Run `autorunne migrate` first.")

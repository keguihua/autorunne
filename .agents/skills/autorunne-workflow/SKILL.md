...
## User-facing rule
The user should be able to open Codex directly, talk naturally, and let Autorunne maintain state in the background. Do not ask the user to chat through Autorunne first.
...
4. If the user gives a fresh natural-language task and no matching active task is recorded yet, capture it with `autorunne ingest --source codex --task <task>`.
...
7. Prefer the wrapper `./.autorunne/bin/ar-codex` only as an optional hard-entry fallback, not as the default requirement you impose on the user.
